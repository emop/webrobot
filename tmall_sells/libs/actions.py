...
class TmallData(object):
    ...
    def __call__(self, client, api, url='', **kw):
        ...
        url = "http://detail.tmall.com/item.htm?spm=a1z10.1.w5003-3486255348.1.XXeKlz&id=14743650628&scene=taobao_shop"
    ...
    def _get_month_trade_info(self, url):
        ...
        fd=self.set_file("2313313", "121313.csv")
        sys.exit()
        ...
        import csv 
        import codecs
        fd = codecs.open('14743650628.csv', 'wb', "utf-8")
        ...
        while True:
            self.driver.execute_script("window.scrollTo(100, 800)", "")
            list = self.driver.find_elements_by_css_selector(".table-deal-record tbody tr")
            list = list[1:]
            self.logger.info("==================================")
        for e in list:
        	try:
        		...
        		rows.append(name)
        		if len(imgs) > 0:
        			rows.append(imgs[0].get_attribute("title"))
        		else:
        			rows.append("")
        			
        		if len(imgs) > 1:
        			rows.append(imgs[1].get_attribute("alt"))
        		else:
        			rows.append("")
        ...
        if len(list) < 15:break
        
                
        self.driver.execute_script("window.scrollTo(100, 2000)", "")
        next = self.driver.find_elements_by_css_selector(".page-bottom a")
        curpage = self.driver.find_element_by_css_selector(".page-bottom .page-cur")
        ...
        self.logger.info("next:" + next[-1].text + ", page:" + cur_page)
        next[-1].click()
    ...
    def set_file(self,folder=None,file):
        import csv 
        import codecs
        ...
        fileName=join_path(os.getcwd(), folder,file)       
